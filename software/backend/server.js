...
process.on('unhandledRejection', (reason, promise) => {
    console.warn('[WARNING] Unhandled Promise Rejection:', reason && reason.message ? reason.message : reason);
});
process.on('uncaughtException', (error) => {
    console.error('[CRITICAL] Uncaught Exception:', error && error.message ? error.message : error);
});
...
function startStorageInit() {
    ...
    storageInitPromise = Promise.resolve()
        ...
        .then(() => {
            ...
            firebaseAdminAvailable = admin.apps.length > 0;
            console.log('[SERVER] Storage init completed');
        })
        .catch((error) => {
            storageInitError = error && error.message ? error.message : String(error);
            console.error('[SERVER] Storage init failed:', storageInitError);
        });
...
async function probeDatabaseHealth(deep = false) {
    if (!deep && typeof storage.getHealthStatus === 'function') {
        return {
            ...storage.getHealthStatus(),
            ...(storageInitError ? { reason: storageInitError } : {})
        };
    }
    ...
    try {
        ...
        return {
            ...
            write: 'not_checked',
            collections: {
                queue: Array.isArray(snapshot.queue) ? snapshot.queue.length : 0,
...
async function ensureStudentLinkedAccount({ email, roll, name }) {
    ...
    if (created) {
        mailer.sendWelcomeEmail(normalizedName, normalizedEmail, 'Sign in using your ERP SSO credentials').catch(mailErr => {
            console.warn('[MAILER] ERP SSO welcome email failed:', mailErr.message);
        });
    }
...
async function resolveStudentForRequest(email) {
    ...
    let student = byEmail || byConfiguredRoll || byInferredRoll;
    if (!student && user && user.role === 'STUDENT') {
        const studentData = {
            roll: String(user.roll || email.split('@')[0]),
        ...
        try {
            ...
            console.error('Self-healing student profile creation failed:', e.message);
        }
    }
...
// ============================================================
// TIMETABLE & ANNOUNCEMENTS ENDPOINTS
// ============================================================
app.get('/api/student/timetable', verifyTokenWithRole(storage), requireRole(['STUDENT']), async (req, res) => {
    try {
        const { student } = await resolveStudentForRequest(req.user.email);
        if (!student || !student.studentClass) {
            return res.json({ success: true, timetable: null, studentClass: null });
        }
        const timetable = await storage.getTimetable(student.studentClass);
        res.json({ success: true, timetable, studentClass: student.studentClass, practicalBatch: student.practicalBatch || null });
    } catch (error) {
...
app.get('/api/student/announcements', verifyTokenWithRole(storage), requireRole(['STUDENT']), async (req, res) => {
    try {
        const { student } = await resolveStudentForRequest(req.user.email);
        const className = student ? student.studentClass : null;
        const announcements = await storage.getAnnouncements(className);
...
app.post('/api/admin/timetable', verifyTokenWithRole(storage), requireRole(['ADMIN', 'GFM']), async (req, res) => {
    try {
        const { studentClass: bodyStudentClass, className, timetableData } = req.body || {};
        const studentClass = bodyStudentClass || className;
        if (!studentClass || !studentClass.trim()) {
            return res.status(400).json({ success: false, message: 'Class name is required' });
        }
        if (!timetableData || typeof timetableData !== 'object') {
            return res.status(400).json({ success: false, message: 'Timetable data is required' });
        }
        await storage.saveTimetable(studentClass.trim(), timetableData);
        await storage.addAuditLog({ event: 'TIMETABLE_SAVED', details: `Class: ${studentClass.trim()} by ${req.user.email}` });
...
app.get('/api/admin/timetable/:className', verifyTokenWithRole(storage), requireRole(['ADMIN', 'GFM']), async (req, res) => {
    try {
        const className = req.params.className;
        const timetable = await storage.getTimetable(className);
...
app.post('/api/admin/announcements', verifyTokenWithRole(storage), requireRole(['ADMIN', 'GFM']), async (req, res) => {
    try {
        const { title, content, targetClass, category } = req.body || {};
        ...
        const announcement = {
            ...
            targetClass: targetClass ? targetClass.trim() : 'ALL',
            category: category ? category.trim() : 'INFO',
            postedBy: req.user.name || req.user.email,
            postedByEmail: req.user.email,
            postedAt: new Date().toISOString()
        };
        await storage.addAnnouncement(announcement);
        await storage.addAuditLog({ event: 'ANNOUNCEMENT_POSTED', details: `Title: ${announcement.title} to ${announcement.targetClass} by ${req.user.email}` });
        res.json({ success: true, announcement, message: 'Announcement posted successfully' });
    } catch (error) {
...
app.get('/api/admin/announcements', verifyTokenWithRole(storage), requireRole(['ADMIN', 'GFM']), async (req, res) => {
    try {
...
app.delete('/api/admin/announcements/:id', verifyTokenWithRole(storage), requireRole(['ADMIN', 'GFM']), async (req, res) => {
    try {
        const id = req.params.id;
        const deleted = await storage.removeAnnouncement(id);
        if (deleted) {
            await storage.addAuditLog({ event: 'ANNOUNCEMENT_DELETED', details: `ID: ${id} by ${req.user.email}` });
...
app.get('/api/health', async (req, res) => {
    ...
    const database = await probeDatabaseHealth(deep);
    ...
    res.json({
        ...
        details: {
            ...
            smtp,
            database,
            healthProbe: deep ? 'deep' : 'lightweight'
        }
    });
});
...
app.get('/api/wifi-config', verifyTokenWithRole(storage), requireRole(['ADMIN', 'GFM']), async (req, res) => {
    try {
        const profiles = await storage.getWifiProfiles();
        res.json({ success: true, profiles });
    } catch (error) {
        console.error('wifi-config GET error:', error);
        res.status(500).json({ success: false, message: 'Server error' });
    }
});

app.post('/api/wifi-config', verifyTokenWithRole(storage), requireRole(['TEACHER', 'ADMIN', 'GFM']), async (req, res) => {
    ...
    try {
        const profile = {
            id: 'wifi_' + crypto.randomBytes(4).toString('hex'),
            ssid: ssid.trim(),
            password: password || '',
        ...
        await storage.addWifiProfile(profile);
        await storage.addAuditLog({ event: 'WIFI_PROFILE_ADDED', details: `SSID: ${profile.ssid} by ${profile.addedBy}` });
        res.json({ success: true, message: 'Wi-Fi profile saved', profile });
    } catch (error) {
        console.error('wifi-config POST error:', error);
        res.status(500).json({ success: false, message: 'Server error' });
    }
});

app.delete('/api/wifi-config/:id', verifyTokenWithRole(storage), requireRole(['ADMIN', 'GFM']), async (req, res) => {
    try {
        const deleted = await storage.removeWifiProfile(req.params.id);
        if (deleted) {
            await storage.addAuditLog({ event: 'WIFI_PROFILE_DELETED', details: `ID: ${req.params.id}` });
            res.json({ success: true, message: 'Wi-Fi profile deleted' });
        } else {
...
app.get('/api/device/config', async (req, res) => {
    ...
    try {
        ...
        // Include Wi-Fi profiles for device auto-connect
        let wifiNetworks = [];
        try {
            const profiles = await storage.getWifiProfiles();
            wifiNetworks = profiles.map(p => ({ ssid: p.ssid, password: p.password || '' }));
        } catch (e) {
            console.warn('[DEVICE_CONFIG] Failed to load wifi profiles:', e.message);
        }
...
app.delete('/api/admin/delete-user', verifyTokenWithRole(storage), requireRole('ADMIN'), async (req, res) => {
    ...
    try {
        ...
        const targetUser = users.find(u => String(u.email || '').toLowerCase().trim() === searchEmail);
        const resolvedEmail = targetUser ? targetUser.email : email;
        ...
        // If user is a student, delete student profile and queue entry
        if (targetUser && (targetUser.role === 'STUDENT' || targetUser.roll)) {
            try {
                const roll = targetUser.roll || resolvedEmail.split('@')[0];
                await storage.removeStudent(roll);
                await storage.removeFromQueue(roll);
            } catch (studentErr) {
                console.warn('[STORAGE] Failed to clean up student profile on user delete:', studentErr.message);
            }
        }

        await storage.deleteUser(resolvedEmail);
        await storage.addAuditLog({ event: 'USER_DELETED', target: resolvedEmail, by: req.user.email });
...
app.post('/api/register-queue', async (req, res) => {
    ...
    try {
        ...
        const existing = queue.find(q => String(q.roll) === String(roll));
        if (existing) {
            ...
            return res.json({ success: true, message: 'Registration updated and reset to PENDING' });
        }
...
app.post('/api/gfm/enrollment-queue/:roll/authorize', verifyTokenWithRole(storage), requireRole(['GFM', 'ADMIN']), async (req, res) => {
    ...
    try {
        const queue = await storage.getQueue();
        const item = queue.find(q => String(q.roll) === String(roll));
        ...
        const active = queue.find(q =>
            (q.status === QUEUE_STATUS.AUTHORIZED || q.status === QUEUE_STATUS.IN_PROGRESS) && String(q.roll) !== String(roll)
        );
        if (active) {
...
// Update student academic details (class/electives/practicals)
app.put('/api/students/:roll/academic-details', verifyTokenWithRole(storage), requireRole(['GFM', 'ADMIN']), async (req, res) => {
    try {
        const { roll } = req.params;
        const { enrolledSubjects, studentClass, practicalBatch } = req.body;
        
        if (enrolledSubjects !== undefined && !Array.isArray(enrolledSubjects)) {
            return res.status(400).json({ success: false, message: 'enrolledSubjects must be an array' });
        }
        
        if (!storage.updateStudentAcademics) {
             return res.status(501).json({ success: false, message: 'Feature not implemented in storage' });
        }

        const updatedStudent = await storage.updateStudentAcademics(roll, {
            enrolledSubjects,
            studentClass,
            practicalBatch
        });
        ...
        await storage.addAuditLog({
            event: 'STUDENT_ACADEMICS_UPDATED',
            user: req.user.email,
            details: `Updated academics for roll ${roll}`
        });

        res.json({ success: true, message: 'Academic details updated successfully', student: updatedStudent });
    } catch (error) {
        console.error('Update student academics error:', error);
        res.status(500).json({ success: false, message: 'Failed to update academic details' });
    }
});
...
app.delete('/api/students/:roll', verifyTokenWithRole(storage), requireRole(['GFM', 'ADMIN']), async (req, res) => {
    const { roll } = req.params;
    try {
        // Find linked user credentials to delete them
        try {
            const students = await storage.getStudents();
            ...
            const users = await storage.getUsers();
            const targetUser = users.find(u => (studentEmail && u.email === studentEmail) || (u.roll && String(u.roll) === String(roll)));

            if (targetUser) {
                // Delete Firebase Auth user if present
                if (AUTH_MODE === 'FIREBASE' && admin.apps.length > 0 && targetUser.uid) {
                    try {
                        const adminAuth = require('firebase-admin').auth();
                        await adminAuth.deleteUser(targetUser.uid);
                    } catch (err) {
                        console.log(`[AUTH] Failed to delete Firebase Auth user for student:`, err.message);
                    }
                }
                // Delete from users collection
                await storage.deleteUser(targetUser.email);
            }
        } catch (credentialsErr) {
            console.warn('[STORAGE] Failed to clean up student user credentials:', credentialsErr.message);
        }
...
app.post('/api/confirm-enrollment', validateDevice, async (req, res) => {
    ...
    try {
        const queue = await storage.getQueue();
        const student = queue.find(q => parseInt(q.roll, 10) === parseInt(roll, 10));
        if (!student) {
        ...
        if (result === 'SUCCESS') {
            ...
            await storage.removeFromQueue(student.roll);
            await storage.addAuditLog({ event: 'ENROLL_SUCCESS', roll: student.roll, fingerprintId: enrolled.fingerprintId });
            res.json({ success: true, message: 'Enrollment confirmed' });
        } else if (result === 'TIMEOUT') {
            await storage.updateQueueItem(student.roll, {
                status: QUEUE_STATUS.FAILED,
                failedAt: new Date().toISOString(),
                failReason: 'Enrollment timed out'
            });
            await storage.addAuditLog({ event: 'ENROLL_TIMEOUT', roll: student.roll });
            res.json({ success: true, message: 'Timeout recorded' });
        } else if (result === 'CANCELLED') {
            await storage.removeFromQueue(student.roll);
            await storage.addAuditLog({ event: 'ENROLL_CANCELLED', roll: student.roll });
            ...
            await storage.addAuditLog({ event: 'ENROLL_FAILED', roll: student.roll });
...
app.post('/api/gfm/enrollment-queue/:id/create-account', verifyTokenWithRole(storage), requireRole(['GFM', 'ADMIN']), async (req, res) => {
    ...
    try {
        ...
        const chosenPassword = password || generateTemporaryPassword();
        let uid = null;
        ...
        if (AUTH_MODE === 'FIREBASE') {
            ...
            try {
                ...
                await admin.auth().updateUser(userRecord.uid, { password: chosenPassword });
            } catch (e) {
            ...
            localPassword = chosenPassword;
        }

        // Send welcome email
        let emailSent = false;
        try {
            emailSent = await mailer.sendWelcomeEmail(item.name, item.email, chosenPassword);
        } catch (mailErr) {
            console.warn('[MAILER] Welcome email failed:', mailErr.message);
        }
        ...
        res.json({
            success: true,
            message: emailSent
                ? (AUTH_MODE === 'LOCAL' ? 'Student account created locally.' : 'Student account created successfully.')
                : (AUTH_MODE === 'LOCAL' ? 'Student account created locally, but welcome email failed.' : 'Student account created successfully, but welcome email failed.'),
            mailSent: emailSent,
            localPassword: chosenPassword,
...
app.post('/api/admin/create-student', verifyTokenWithRole(storage), requireRole('ADMIN'), async (req, res) => {
    const { email, roll, name, password } = req.body;
    if (!email || !roll || !name) {
    ...
    try {
        ...
        let emailSent = false;
        try {
            emailSent = await mailer.sendWelcomeEmail(name, email, chosenPassword);
        } catch (mailErr) {
            console.warn('[MAILER] Welcome email failed:', mailErr.message);
        }
...
// ============================================================
// LOW ATTENDANCE WARNING HELPERS & ENDPOINTS
// ============================================================

function calculateStudentAttendanceStats(student, attendance, apps) {
    const studentRoll = String(student.roll);
    const myLogs = attendance
        .filter(rec => String(rec.roll) === studentRoll)
        .sort((a, b) => new Date(b.timestamp || 0) - new Date(a.timestamp || 0));
...
async function getStudentsBelowThreshold(threshold = 75) {
    const students = await storage.getStudents();
    const attendance = await storage.getAttendance();
    const apps = await storage.getApplications();
    ...
    for (const student of students) {
        ...
        const subjects = calculateStudentAttendanceStats(student, attendance, apps);
        const lowSubjects = subjects
            .filter(sub => sub.percent < threshold)
            .map(sub => ({
                subject: sub.subject,
...
// GFM: MANUALLY TRIGGER ATTENDANCE WARNINGS
app.post('/api/gfm/send-attendance-warnings', verifyTokenWithRole(storage), requireRole(['GFM', 'ADMIN']), async (req, res) => {
    try {
        const threshold = 75;
        const lowAttendanceList = await getStudentsBelowThreshold(threshold);
        let warningCount = 0;
        const warnedStudents = [];

        for (const item of lowAttendanceList) {
            const { student, lowSubjects } = item;
            console.log(`[MANUAL WARNING] Sending low-attendance warning to student ${student.name} (${student.email})`);
            const emailSent = await mailer.sendLowAttendanceWarningEmail(student.name, student.email, lowSubjects, threshold);
            if (emailSent) {
                warningCount++;
                warnedStudents.push({ name: student.name, roll: student.roll, email: student.email });
                await storage.updateStudentAcademics(student.roll, { lastWarningSent: new Date().toISOString() });
            }
        }

        await storage.addAuditLog({ 
            event: 'MANUAL_ATTENDANCE_WARNINGS_SENT', 
            by: req.user.email, 
            details: `Sent warnings to ${warningCount} students` 
        });

        res.json({
            success: true,
            message: `Sent low attendance warnings to ${warningCount} students.`,
            warnedStudents
        });
    } catch (error) {
        console.error('manual-warnings error:', error);
        res.status(500).json({ success: false, message: 'Server error' });
    }
});
    ...
    setInterval(async () => {
        try {
            ...
            if (currentHour === 20 && lastReportDate !== currentDateStr) {
                console.log(`[SCHEDULER] Running daily attendance tasks for ${currentDateStr}...`);

                // 1. Run low attendance warning checks for students
                try {
                    const lowAttendanceList = await getStudentsBelowThreshold(75);
                    let warningCount = 0;
                    ...
                    for (const item of lowAttendanceList) {
                        ...
                        const lastSent = student.lastWarningSent ? new Date(student.lastWarningSent).getTime() : 0;
                        if (nowMs - lastSent >= oneWeekMs) {
                            console.log(`[SCHEDULER] Sending low-attendance warning to student ${student.name} (${student.email})`);
                            const emailSent = await mailer.sendLowAttendanceWarningEmail(student.name, student.email, lowSubjects, 75);
                            if (emailSent) {
                                warningCount++;
                                await storage.updateStudentAcademics(student.roll, { lastWarningSent: new Date().toISOString() });
                            }
                        } else {
                            console.log(`[SCHEDULER] Cooldown active for student ${student.name} (${student.email}). Skipping warning email.`);
                        }
                    }
                    console.log(`[SCHEDULER] Completed daily warnings check. Sent ${warningCount} warning emails.`);
                } catch (warnErr) {
                    console.error('[SCHEDULER] Error in daily student warnings check:', warnErr.message);
                }

                // 2. Run daily teacher report
                ...
                if (todayAttendance.length > 0) {
                    ...
                    console.log('[SCHEDULER] No attendance sessions logged today. Skipping email summaries for teachers.');
                }
...
async function startServer() {
    try {
        const httpServer = app.listen(PORT, '0.0.0.0', () => {
            ...
            startStorageInit()
                .then(() => {
                    const health = typeof storage.getHealthStatus === 'function' ? storage.getHealthStatus() : { status: 'ok' };
                    if (health.status !== 'ok') {
                        console.warn('[SERVER] Startup audit log skipped because storage is degraded.');
                        return null;
                    }
                    return Promise.resolve(storage.addAuditLog({ event: 'SERVER_START', port: PORT, storageMode: STORAGE_MODE, authMode: AUTH_MODE }));
                })
                .catch((error) => {
